...
<Callout type="info">
  Files starting with underscore (`_`) are special Next.js/Nextra files and won't be rendered as
  pages.
</Callout>
...
<Callout type="warning">
  Never skip heading levels. Always go from H2 → H3 → H4, not H2 → H4.
</Callout>
...
<Callout type="error">Error or danger message</Callout>
...
<Tabs items={['npm', 'yarn', 'pnpm']}>
  <Tabs.Tab>```bash npm install mimir-code ```</Tabs.Tab>
  <Tabs.Tab>```bash yarn add mimir-code ```</Tabs.Tab>
  <Tabs.Tab>```bash pnpm add mimir-code ```</Tabs.Tab>
</Tabs>
````
...
<Callout>Also known as: setup, getting started, download</Callout>
```
...
Go to Settings → Pages → Build and deployment → GitHub Actions
