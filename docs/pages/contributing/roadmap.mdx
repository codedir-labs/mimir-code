...
**See**: `docs/pages/contributing/TEAMS-ARCHITECTURE.md` for detailed architecture
...
- [ ] **OpenAI** (Priority 1): API integration, model selection, cost calc, streaming (OpenAI-compatible)
- [ ] **Google/Gemini** (Priority 2): API integration, Gemini SDK, model selection, cost calc
- [ ] **Azure OpenAI** (Priority 3): Enterprise provider, model selection, cost calc
- [ ] **Qwen** (Priority 4): OpenAI-compatible, similar to DeepSeek
- [ ] **Ollama (Local)** (Priority 5): Local API, no cost, model management
- [ ] **Amazon Bedrock** (Priority 6): Enterprise provider, multiple models
- [ ] **Google Vertex AI** (Priority 7): Enterprise GCP provider
...
### Provider Authentication Abstraction (OAuth/Account-based)

- [ ] **Authentication Layer**: Abstract auth for providers (API keys vs OAuth)
- [ ] **OAuth Flow**: Browser-based OAuth for providers that support it
- [ ] **Anthropic Account Auth**: `/connect anthropic` to authenticate via Anthropic account
- [ ] **Google/Gemini Auth**: OAuth flow for Google Cloud credentials
- [ ] **Azure Auth**: Microsoft OAuth for Azure OpenAI
- [ ] **Token Management**: Secure storage, refresh logic, expiration handling
- [ ] **Fallback to API Keys**: Support both auth methods (OAuth preferred, API key fallback)
- [ ] **Teams Mode**: Route auth through Teams API (hide individual accounts)

### Network & Proxy Support

- [ ] **Proxy Configuration**: HTTPS_PROXY, HTTP_PROXY, NO_PROXY environment variables
- [ ] **Custom Certificates**: NODE_EXTRA_CA_CERTS for corporate proxies
- [ ] **Proxy Authentication**: Basic auth, NTLM/Kerberos via gateway
- [ ] **Network Diagnostics**: Test proxy connectivity in `mimir doctor`

### Small Model Optimization

- [ ] **Small Model Config**: Separate `smallModel` setting for lightweight tasks
- [ ] **Task Complexity Detection**: Automatically detect simple vs complex tasks
- [ ] **Automatic Switching**: Use Haiku/GPT-4o-mini for title generation, summaries, etc.
- [ ] **Cost Optimization**: Reduce costs by routing simple tasks to cheaper models
...
**See**: `docs/pages/contributing/plan-tools.md` for detailed architecture
...
- [ ] **Formatter Detection**: Auto-detect formatters in project (prettier, eslint, biome, ruff, gofmt, etc.)
- [ ] **Auto-Run After Writes**: Automatically format files after write/edit operations
...
- [ ] **Configuration**: Enable/disable formatters globally or per-formatter
- [ ] **Custom Formatters**: Support custom formatter commands with `$FILE` placeholder
...
- [ ] **EDITOR Environment Variable**: Support for `$EDITOR` (VS Code, Vim, Nano, etc.)
- [ ] `/editor` Command: Open external editor for composing long prompts
- [ ] **Editor Integration**: `code --wait`, Cursor, Vim, Nano support
- [ ] **File-based Composition**: Write prompt to temp file, read after editor closes

### Command Palette

- [ ] **Command Palette UI**: Ctrl+P (configurable) to open command palette
- [ ] **Fuzzy Search**: Search for slash commands, tools, settings
- [ ] **Keyboard-first Navigation**: Arrow keys, Enter to select
- [ ] **Recent Commands**: Show recently used commands first
- [ ] **Help Text**: Display command descriptions inline
...
**See**: `docs/pages/contributing/plan-agent-orchestration.md` for detailed architecture
...
## Phase 13: GitHub/GitLab Integration & Auto-Update

**Goal**: CI/CD integration and auto-update system

### GitHub Integration

- [ ] **GitHub Actions Workflow**: `.github/workflows/mimir.yml` template
- [ ] **Trigger on Issues/PRs**: Mention `/mimir` in comments to trigger
- [ ] **Code Review Comments**: Comment on specific lines to request fixes
- [ ] **Automatic PR Creation**: Create PRs with fixes automatically
- [ ] **Issue Triage**: Analyze issues and provide context
- [ ] **Custom Prompts**: Override default behavior with custom review criteria

### GitLab Integration

- [ ] **GitLab CI/CD Component**: Community-maintained component integration
- [ ] **GitLab Duo Integration**: Mention `@mimir` in comments
- [ ] **Merge Request Automation**: Automatic MR creation and updates
- [ ] **Pipeline Integration**: Run Mimir in GitLab pipelines

### Auto-Update System

- [ ] **Background Update Checks**: Check for new versions daily
- [ ] **Update Modes**: `true` (auto-download), `false` (disabled), `"notify"` (notify only)
- [ ] **One-Click Updates**: `mimir upgrade` with confirmation
- [ ] **Release Notes Display**: Show release notes before updating
- [ ] **Rollback Support**: `mimir rollback` to previous version
...
**Goal**: Community extensibility with event-driven plugins

**See**: `.claude/outputs/architecture/plugin-system-design.md` for detailed architecture

### Core Plugin Infrastructure

- [ ] **Plugin Loader**: Load plugins from disk/npm
- [ ] **Plugin Validator**: Validate manifest and structure
- [ ] **Plugin Registry**: Register/unregister plugins
- [ ] **Event Bus**: Emit, subscribe, unsubscribe (24 events)
- [ ] **Plugin Context API**: Rich context object (fs, exec, config, llm, session, tools)
- [ ] **V8 Isolate Sandbox**: Isolate plugin execution

### Event System (24 Events)
...
- [ ] **`@mimir/plugin` NPM Package**: Plugin development SDK
- [ ] **TypeScript Types**: Full type definitions for plugin API
- [ ] **Tool Builder**: `tool()` helper for registering custom tools
- [ ] **Plugin Development Docs**: Comprehensive guide with examples
- [ ] **Example Plugins** (5+):
  - Auto-formatter plugin
  - Budget alert plugin
  - Security audit plugin
  - PreCompact migration plugin (built-in)
  - GitHub PR comment plugin

### Plugin Distribution

- [ ] **NPM Distribution**: `mimir-plugin-*` naming convention
- [ ] **Plugin CLI Commands**:
  - `mimir plugin list` - List installed plugins
  - `mimir plugin install <name>` - Install from NPM
  - `mimir plugin enable <name>` - Enable plugin
  - `mimir plugin disable <name>` - Disable plugin
  - `mimir plugin uninstall <name>` - Uninstall plugin
  - `mimir plugin info <name>` - Show plugin details
- [ ] **Plugin Marketplace**: GitHub-based plugin registry
- [ ] **Plugin Discovery UI**: Browse and install plugins
- [ ] **Security Review Process**: For official plugins

### Security & Capabilities

- [ ] **Capability-Based Security**: Explicit permissions in manifest
- [ ] **Plugin Manifest**: `plugin.yml` with capabilities, events, metadata
- [ ] **Resource Limits**: CPU, memory, execution time per plugin
- [ ] **Audit Logging**: Log all plugin actions
- [ ] **Teams Enforcement**: Enterprise plugin allowlists/blocklists

### Teams Integration

- [ ] **Organization-wide Plugins**: Distribute plugins to all team members
- [ ] **Plugin Allowlist Enforcement**: Teams can restrict plugins
- [ ] **Required Plugins**: Force installation of compliance plugins
- [ ] **Plugin Usage Analytics**: Track plugin usage via Teams API

### Migration & Compatibility

- [ ] **PreCompact Hook → Plugin**: Implement PreCompact as built-in plugin
...
- [ ] Event emission from features
- [ ] Plugin interception and modification
...
### v1.1 (Quick Wins - 4-6 weeks)

1. **Auto-Formatting Support** (Phase 4 extension) - 1 week
2. **External Editor Support** (Phase 9 extension) - 3 days
3. **Proxy & Certificate Support** (Phase 3 extension) - 1 week
4. **Small Model Optimization** (Phase 3 extension) - 1 week
5. **Command Palette** (Phase 9 extension) - 1 week
6. **Model Switching & Context** (Phase 11) - 2 weeks
7. **Custom Commands & Checkpoints** (Phase 12) - 2-3 weeks

**Total**: ~7-9 weeks (1.5-2 months)

### v1.2 (Ecosystem & Integration - 6-8 weeks)

1. **GitHub/GitLab Integration** (Phase 13) - 3-4 weeks
2. **Auto-Update System** (Phase 13) - 1 week
3. **Plugin System** (Phase 14) - Phase 1-3 only - 5-7 weeks

**Total**: ~9-12 weeks (2-3 months)

### v2.0 (Mature Ecosystem - Ongoing)

1. **Plugin Marketplace** (Phase 14 - Phase 4) - 3-4 weeks
