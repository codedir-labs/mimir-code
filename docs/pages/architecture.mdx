...
<Tabs items={['Development', 'Build', 'Binary']}>
  <Tab>```bash # Run in development mode yarn dev ```</Tab>
  <Tab>```bash # Build TypeScript to dist/ yarn build ```</Tab>
  <Tab>```bash # Create platform-specific executables yarn build:binary ```</Tab>
</Tabs>
