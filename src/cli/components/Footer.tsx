...
export const Footer: React.FC<FooterProps> =
  ({ theme, shortcuts, mode, cost, interruptPressCount = 0, isAgentRunning = false }) => {
    ...
    const getMessage = () => {
      if (interruptPressCount === 1) {
        ...
        if (isAgentRunning) {
          return <Text>{themeDefinition.colors.warning(`Agent interrupted. Press ${interruptKey} again to exit`)}</Text>;
        } else {
    ...
    return (
      <Box flexDirection="column">
        ...
        <Box>
          <Text dimColor>
            {formatKeyboardShortcut(shortcuts.interrupt)}=Cancel | {formatKeyboardShortcut(shortcuts.modeSwitch)}=Mode | /help=Commands
          </Text>
        </Box>
        <Box>
          {getMessage()}
